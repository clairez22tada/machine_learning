...
def loss_ridge(X, y, B, lmbda):
    return np.sum((y- np.dot(X, B))**2) + lmbda * np.sum(B**2)

def loss_gradient_ridge(X, y, B, lmbda):
    return -np.dot(X.T, y - np.dot(X, B)) + lmbda*np.sum(B)
...
#                predictions.append(0)
    ...
    def fit(self, X, y):
         self.B = minimize(X, y, log_likelihood_gradient, self.eta, self.lmbda, self.max_iter)
...
#        X = np.hstack([np.ones([X.shape[0],1],),X])
        self.B = minimize(X, y, loss_gradient_ridge, self.eta, self.lmbda, self.max_iter, addB0=False)
        self.B0 = np.mean(y)-np.mean(np.dot(X,self.B))
